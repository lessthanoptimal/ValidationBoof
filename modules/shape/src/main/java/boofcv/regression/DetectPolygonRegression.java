...
public class DetectPolygonRegression<T extends ImageGray<T>> extends BaseRegression implements ImageRegression {
    ...
    public void process(ImageDataType type) throws IOException {
        final Class<T> imageType = ImageDataType.typeToSingleClass(type);
    ...
    private void process(String name, boolean localBinary, FactoryObject<DetectPolygonBinaryGrayRefine<T>> factory)
            throws IOException {
        ...
        var outputAccuracy = new PrintStream(new File(directoryMetrics, outputAccuracyName));
        ...
        for (File f : files) {
            ...
            var detection = new DetectPolygonsSaveToFile<>(factory.newInstance(), localBinary);
        ...
        outputAccuracy.println();
        outputAccuracy.printf("Final Summary: TP/(TP+FN) = %d / %d    FP = %d\n%n", totalTruePositive, totalExpected, totalFalsePositive);
    ...
    public static void main(String[] args)
        ...
        RegressionRunner.main(new String[]{DetectPolygonRegression.class.getName(), ImageDataType.U8.toString()});
    }
}
