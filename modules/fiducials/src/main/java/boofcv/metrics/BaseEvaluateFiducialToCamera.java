...
public abstract class BaseEvaluateFiducialToCamera implements FiducialEvaluateInterface {
    ...
    // Where the images and ground truth is stored
    File dataSetDir;
    ...
    protected void initializeEvaluate(File dataSetDir) {
        ...
        expected = checkLoadVisible(new File(dataSetDir, "visible.txt"));
        if (expected == null)
            throw new RuntimeException("Failed to load default 'visible.txt'");
        ...
        for (int i = 0; i < expected.length; i++) {
            detectedCorners[i] = new ArrayList<>();
            fiducialNormal[i] = new Vector3D_F64();
            fiducialPose[i] = new Se3_F64();
        }
    }

    private @Nullable long[] checkLoadVisible(File file ) {
        if (!file.exists())
            return null;
        List<String> visible = FiducialCommon.parseVisibleFile(file);
        long[] expected = new long[visible.size()];
    ...
    protected void evaluate(String fileName, List<FiducialCommon.Detected> detected,
        ...
        long[] imageExpected = checkLoadVisible(new File(dataSetDir, FilenameUtils.getBaseName(fileName)+"_visible.txt"));
        if (imageExpected == null)
            imageExpected = expected;

        totalExpected += imageExpected.length;
        ...
        for (int i = 0; i < detected.size(); i++) {
            ...
            Assignment match = findBestAssignment(corners, truthCorners, imageExpected);
            if (match == null) {
        ...
        for (int i = 0; i < imageExpected.length; i++) {
            if (fiducialDetected[i] == 0) {
                if (!justSummary)
                    outputResults.println(fileName + " false negative for ID " + imageExpected[i]);
                totalFalseNegative++;
            }
    ...
    protected static List<List<Point2D_F64>> extractIndividual(List<Point2D_F64> truthCorners, int totalFiducials) {
        List<List<Point2D_F64>> ret = new ArrayList<>();
    ...
    private Assignment findBestAssignment(List<Point2D_F64> corners,
                                          List<List<Point2D_F64>> truthFiducials, long[] expected) {
