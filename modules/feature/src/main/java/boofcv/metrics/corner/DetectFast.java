...
public class DetectFast {
	...
	public static <T extends ImageGray<T>> void detect( String outputDirectory, Class<T> imageType ) throws FileNotFoundException {
		...
		var found = new QueueCorner();
		alg.getCandidatesHigh().copyInto(found);
		alg.getCandidatesLow().copyInto(found);

		var fos = new PrintStream(outputDirectory + FILE_NAME);
		...
		NonMaxSuppression nonmax = FactoryFeatureExtractor.nonmax(new ConfigExtract(1, 0, 3, true, false, true), 1);
